...
export default function Home() {
  ...
  const createIcoAta = async()=>{
    try {
      ...
      alert(`Error initializing ICO: ${error.message}`);
    }
    finally {
      setLoading(false);
    }
  ...
  const depositIco = async () => {
    try {
      ...
      await program.methods
        ...
        .rpc();
      alert("ICO deposit successful!");
      await fetchIcoData();
    } catch (error) {
      console.error("Error depositing ICO:", error);
      alert(`Error depositing ICO: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const buyTokens = async () => {
    try {
      ...
      if (balance < solCost * 1e9 + 5000) {
        alert(`Insufficient SOL balance. You need at least ${solCost.toFixed(3)} SOL plus transaction fees.`);
        return;
      }
      ...
      console.log("Error purchasing ICO:", error);
      alert(`Error purchasing ICO: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchUserTokenBalance = async () => {
    try {
      ...
      const userIcoAta = await getAssociatedTokenAddress(ICO_MINT, wallet.publicKey);
      try {
        const tokenAccount = await getAccount(connection, userIcoAta);
      ...
      console.log("Error fetching user token balance:", error);
      setUserTokenBalance("0");
    }
  };
            ...
            <div className="max-w-md mx-auto">
             <div className="pb-8">
              <div className="flex justify-between items-center">
                ...
                <WalletMultiButton className="bg-blue-500 text-white px-4 py-2 rounded-lg" />
              </div>
              {
                wallet.connected && (
                    ...
                    <div className="py-8">
                           ...
                           text-sm">
                            ...
                            <div>
                              <div className="text-gray-600">Available</div>
                                <p className="font-medium">
                                  {(icoData.totalTokens - icoData.tokensSold).toString()}{" "} tokens
                                </p>
                            </div>
                            <div>
                      ...
                      <div className="space-y-4">
                        <input
                          ...
                          focus:right-2 focus:ring-blue-500 focus:border-l-blue-500"
                          min="1"
                          step="1"
                        />

                        {
                          amount && !isAdmin && (
                            <div className="p-4 bg-blue-50
                             rounded-lg border border-blue-100 space-y-2">
